...
describe("RoomView", () => {
    it("displays the room name and room code", () => {
        ...
        expect(screen.getByRole("heading", { name: "Study Group", level: 1 })).toBeInTheDocument();
    ...
    it("shows the empty-transcript placeholder before anything has been said", () => {
        mockUseRoomSocket.mockReturnValue(socketState());

        render(<RoomView membership={listenerMembership} onLeave={vi.fn()} />);

        expect(screen.getByText("The transcript will appear here.")).toBeInTheDocument();
    });

    it("also shows the live transcript to the speaker", () => {
        mockUseRoomSocket.mockReturnValue(
            socketState({
                transcript: [{ segmentId: "a", text: "Testing testing", isFinal: true, updatedAt: 1 }],
            })
        );
